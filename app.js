...
var included_sources = [ "allegheny", "austin", "baltimore", "bloomington", "boston", "boulder", "chapelhill", "chatham", "chicago", "clark", "honolulu", "kitsap", "lancaster", "midland", "midlandtx", "nanaimo", "nyc", "oakland", "petaluma", "philadelphia", "pittsburgh", "raleigh", "roundrock", "sagis", "sanfrancisco", "savannah", "seattle", "smith", "spokane", "steamboatsprings", "tyler", "westsacramento" ];
...
app.get('/regions/recent', function(req, res){
  ...
  customgeo.CustomGeo.find({'created': { "$gt": yesterday }}).limit(20).exec(function(err, recents){
    ...
    if(recents.length < 10){
      ...
      customgeo.CustomGeo.find({ 'created': { "$gt": lastmonth } }).sort('-created').limit(10).exec(function(err, recents){
        if(err){ return res.send(err); }
